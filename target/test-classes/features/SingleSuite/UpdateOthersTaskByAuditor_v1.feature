Feature: Verify the functionality of Update Others Task by Refiller
  ...
  @Excel
  Scenario: Update all the Others Task by Auditor from v1 endpoint
    And Give the Excel file to get the request payload for updating Others task "QuickFill_TestData.xlsx"
    And Give the sheet name "UpdateOthersTasksByAuditor" to get all the Others Task to be updated
  ...
  @UOTR
  Scenario: Update OTHERS Task by Refill Auditor with valid data from v1 endpoint
    And Get the refillId to update others task from v1 endpoint
    And Give the status to update the others task from v1 endpoint "VERIFIED"
    And Give the comments for updating others task by auditor from v1 endpoint "checking the update others task functionality"
...
#  @UOTR
  Scenario: Update OTHERS Task by Refill Auditor without refillID from v1 endpoint
    And Give the status to update the others task from v1 endpoint "VERIFIED"
    And Give the comments for updating others task by auditor from v1 endpoint "checking the update others task functionality"
    When Requester calls the Update Others Task v1 endpoint to update others task
...
#  @UOTR
  Scenario: Update OTHERS Task by Refill Auditor without status from v1 endpoint
    And Get the refillId to update others task from v1 endpoint
    And Give the comments for updating others task by auditor from v1 endpoint "checking the update others task functionality"
    When Requester calls the Update Others Task v1 endpoint to update others task
    Then Verify that status code be equal to "400"
...
#  @UOTR
  Scenario Outline: Update OTHERS Task by Refill Auditor with invalid refillId from v1 endpoint
...
#  @UOTR
  Scenario: Update OTHERS Task by Refill Auditor of other siteID from v1 endpoint
    And Get the refillId to update others task from v1 endpoint
    And Give the status to update the others task from v1 endpoint "VERIFIED"
    And Give the comments for updating others task by auditor from v1 endpoint "checking the update others task functionality"
    When Requester calls the Update Others Task v1 endpoint to update others task
